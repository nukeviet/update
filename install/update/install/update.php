		...
		if( $nv_Request->isset_request( 'move', 'get' ) )
		{
			...
			if( $check === true )
			{
				...
				try
				{
					...
					$array_admin['checknum'] = md5( $array_admin['checknum'] );
					$admin_id = intval( $array_admin['admin_id'] );
					$admin_serialize = serialize( $array_admin );

					$sth = $db->prepare( 'UPDATE ' . NV_AUTHORS_GLOBALTABLE . ' SET check_num = :check_num WHERE admin_id=' . $admin_id );
					$sth->bindValue( ':check_num', $array_admin['checknum'], PDO::PARAM_STR );
					$sth->execute( );
...
{
	if ( $global_config['version'] == '4.0.10')
	{
		nv_save_file_config_global( );
		Header( 'Location:' . NV_BASE_SITEURL . 'install/update.php?step=3' );
		exit( );
